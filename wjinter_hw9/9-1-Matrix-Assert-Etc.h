...
public:
	...
	double determinant() const {
	    static_assert(rows == cols, "only determinant for square matrices");
		double val = 0;
		for (int i = 0; i < rows; i++) {
			val += (i % 2 ? -1 : 1)
				* data[i][0]
...
{
    static_assert(b == c, "only determinant for square matrices");
	Matrix<a, c> result;
	for (int i = 0; i < a; i++) {
		for (int j = 0; j < c; j++) {
			double total = 0;
...
//fixed the problem
//before check Straight-Result.png
//After check Fixed-Result.png
